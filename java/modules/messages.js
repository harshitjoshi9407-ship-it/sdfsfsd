...
const downloadMessageMedia = async (client, message, mediaPath, fileIndex = 1, totalFiles = 1, options = {}) => {
  ...
  try {
    ...
    if (message.media) {
      ...
      // Get file size for adaptive optimization (use largest photo size if available)
      ...
      const fileSizeMB = estimatedSize / (1024 * 1024);
      
      // Adaptive chunk sizing based on file size - compliant with Telegram API limits
      let adaptiveChunkSize;
      if (fileSizeMB < 100) {
        adaptiveChunkSize = 1 * 1024 * 1024; // 1MB for small files - faster startup
      } else if (fileSizeMB < 500) {
        adaptiveChunkSize = 2 * 1024 * 1024; // 2MB for medium files
      } else {
        adaptiveChunkSize = 4 * 1024 * 1024; // 4MB for large files - max safe Telegram chunk
      }
      ...
      const downloadSettings = isSingleFile ? {
        ...
        chunkSize: adaptiveChunkSize, // Full adaptive chunk size for single file
        ...
        chunkSize: Math.min(adaptiveChunkSize, 4 * 1024 * 1024), // Adaptive chunks capped at 4MB for batch stability
